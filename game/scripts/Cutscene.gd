...
## A gift Poké Ball containing a Pokémon (Eevee). Gives it, flags it taken, hides the ball.
func gift_mon_ball(species: String, level: int, got_event: String, npc) -> void:
	main.cutscene_active = true
	main.modal = null
	if not _receive_mon(species, level):
		await say("Your party and\nbox are full!")
		main.cutscene_active = false
		return
	if main.audio:
		main.audio.play_sfx("get_item1")
	await say("%s received\na %s!" % [main.player_name, species.to_upper()])
	await offer_nickname(_last_received)
	main.set_event(got_event)
	if npc:
		npc.set_shown(false)
	main.cutscene_active = false


## The Fighting Dojo Hitmon prize (scripts/FightingDojo.asm): pick ONE; the other vanishes.
func hitmon_gift(which: String, npc) -> void:
	main.cutscene_active = true
	main.modal = null
	if main.has_event("GOT_HITMONLEE") or main.has_event("GOT_HITMONCHAN"):
		await say("Better not get\ngreedy...")
		main.cutscene_active = false
		return
	var desc := "hard kicking\nHITMONLEE" if which == "hitmonlee" else "piston punching\nHITMONCHAN"
	if not await ask("You want the\n%s?" % desc):
		main.cutscene_active = false
		return
	if not _receive_mon(which, 30):
		await say("Your party and\nbox are full!")
		main.cutscene_active = false
		return
	if main.audio:
		main.audio.play_sfx("get_item1")
	await say("%s received\na %s!" % [main.player_name, which.to_upper()])
	await offer_nickname(_last_received)
	main.set_event("GOT_" + which.to_upper())
	main.set_event("DEFEATED_FIGHTING_DOJO")
	if npc:
		npc.set_shown(false)
	var other_key := "SPRITE_POKE_BALL@5,1" if which == "hitmonlee" else "SPRITE_POKE_BALL@4,1"
	var other = main._npc_by_key(other_key)
	if other:
		other.set_shown(false)
	main.cutscene_active = false


## The Mt. Moon Pokécenter MAGIKARP salesman (scripts/MtMoonPokecenter.asm): L5 for ¥500.
func magikarp_salesman() -> void:
	main.cutscene_active = true
	main.modal = null
	if main.has_event("BOUGHT_MAGIKARP"):
		await say("MAN: How's that\nMAGIKARP?\fGood, isn't it?")
		main.cutscene_active = false
		return
	main.moneybox.show_box()                  # MONEY_BOX before the sales pitch (MtMoonPokecenter.asm)
	if not await ask("MAN: Hello, there!\nHave I got a deal\njust for you!\fI'll let you have\na swell MAGIKARP\nfor just ¥500!\fWhat do you say?"):
		await say("No? I'm only\ndoing this as a\nfavor to you!")
		main.moneybox.hide_box()
		main.cutscene_active = false
		return
	if main.player_money < 500:
		await say("You'll need more\nmoney than that!")
		main.moneybox.hide_box()
		main.cutscene_active = false
		return
	if not _receive_mon("magikarp", 5):
		await say("Your party and\nbox are full!")
		main.moneybox.hide_box()
		main.cutscene_active = false
		return
	...
	if main.audio:
		main.audio.play_sfx("get_item1")
	await say("%s received\na MAGIKARP!" % main.player_name)
	main.moneybox.hide_box()                  # the naming screen clears the tilemap on the GB
	await offer_nickname(_last_received)
	main.set_event("BOUGHT_MAGIKARP")
	main.cutscene_active = false
...
## A Mt. Moon fossil (scripts/MtMoonB2F.asm): take DOME or HELIX; you only get one.
func mtmoon_fossil(item: String, got: String, npc, other_key: String) -> void:
	main.cutscene_active = true
	main.modal = null
	if main.has_event("GOT_DOME_FOSSIL") or main.has_event("GOT_HELIX_FOSSIL"):
		await say("You already took\none. Don't be\ngreedy!")
		main.cutscene_active = false
		return
	if not await ask("You want the\n%s?" % item):
		main.cutscene_active = false
		return
	if not await _gift(item):
		return
	if main.audio:
		main.audio.play_sfx("get_key_item")
	await say("%s took the\n%s!" % [main.player_name, item])
	main.set_event(got)
	if npc:
		npc.set_shown(false)
	var other = main._npc_by_key(other_key)         # the other fossil is now out of reach
	if other:
		other.set_shown(false)
	main.cutscene_active = false


## The Pewter Museum scientist (scripts/Museum1F.asm) hands over the OLD AMBER.
func give_old_amber() -> void:
	main.cutscene_active = true
	main.modal = null
	if main.has_event("GOT_OLD_AMBER"):
		await say("Ssh! Get the OLD\nAMBER checked!")
		main.cutscene_active = false
		return
	await say("Ssh! I think that\nthis chunk of\nAMBER contains\nPOKéMON DNA!"
		+ "\fIt would be great\nif POKéMON could\nbe resurrected\nfrom it!"
		+ "\fBut, my colleagues\njust ignore me!\fSo I have a favor\nto ask!"
		+ "\fTake this to a\nPOKéMON LAB and\nget it examined!")
	if not main.add_item("OLD AMBER"):
		await say("You don't have\nspace for this!")
		main.cutscene_active = false
		return
	if main.audio:
		main.audio.play_sfx("get_item1")
	await say("%s received\nOLD AMBER!" % main.player_name)
	main.set_event("GOT_OLD_AMBER")
	var amber = main._npc_by_key("SPRITE_OLD_AMBER@16,2")
	if amber:
		amber.set_shown(false)
	main.cutscene_active = false


## The museum receptionist (scripts/Museum1F.asm Museum1FScientist1Text): a ¥50 child's
## ticket gates the exhibit. Refusal — or empty pockets — turns you back a step south
## (pokered simulates PAD_DOWN); with a ticket she just waves you through.
func museum_ticket() -> void:
	main.cutscene_active = true
	main.modal = null
	if main.has_event("BOUGHT_MUSEUM_TICKET"):
		await say("Take plenty of\ntime to look!")
		main.cutscene_active = false
		return
	main.moneybox.show_box()                  # MONEY_BOX before the entry prompt (Museum1F.asm)
	if await ask("It's ¥50 for a\nchild's ticket.\fWould you like to\ncome in?"):
		if main.player_money >= 50:
			main.player_money -= 50
			main.moneybox.refresh()           # redrawn right after SubBCDPredef
			if main.audio:
				main.audio.play_sfx("purchase")
			await say("Right, ¥50!\nThank you!")
			main.set_event("BOUGHT_MUSEUM_TICKET")
			main.moneybox.hide_box()
			main.cutscene_active = false
			return
		await say("You don't have\nenough money.")
	await say("Come again!")
	main.moneybox.hide_box()
	main.player.step(DOWN)                    # turned back off the counter aisle
	main.cutscene_active = false


## Chatting with the receptionist from behind the counter (.behind_counter): the amber
## trivia yes/no — YES gets the lab hint, NO the tree-sap explanation.
func museum_amber_chat() -> void:
	main.cutscene_active = true
	main.modal = null
	if await ask("You can't sneak\nin the back way!\fOh, whatever!\nDo you know what\nAMBER is?"):
		await say("There's a lab\nsomewhere trying\nto resurrect\nancient POKéMON\nfrom AMBER.")
	else:
		await say("AMBER is fossil-\nized tree sap.")
	main.cutscene_active = false
...
# ---- Silph Co (scripts/SilphCo7F.asm, SilphCo11F.asm) -----------------------

# Player starter is irrelevant; the Silph rival party is keyed by the rival's starter.
const _SILPH_RIVAL_PARTY := {"squirtle": 7, "bulbasaur": 8, "charmander": 9}


## The rival waited for you on Silph Co 7F (OPP_RIVAL2, the strongest mid-game team).
func silph_rival() -> void:
	main.cutscene_active = true
	main.modal = null
	var rn: String = main.rival_name
	var pn: String = main.player_name
	var at_lower: bool = main.player.cell == Vector2i(3, 3)
	# SilphCo7FDefaultScript: the player turns as MEET_RIVAL strikes, he calls out, then walks
	# up from the corridor to meet you; after the battle he wishes you luck facing up, and
	# leaves toward the elevator with the jingle re-struck.
	main.player.face(DOWN)
	var rival = main._npc_by_key("SPRITE_BLUE@3,7")
	if main.audio:
		main.audio.play_song("meetrival")
	await say("%s: Hahaha!\nI thought you'd\nturn up if I\nwaited here!\fI saw you in\nSAFFRON, so I\ndecided to see if\nyou got better!" % rn)
	if rival:
		# scripts/SilphCo7F.asm RivalMovementUp: straight up column 3, stopping below RED.
		# player_can_enter does not include player occupancy, so target that cell explicitly.
		var approach_target: Vector2i = Vector2i(3, main.player.cell.y + 1)
		var approach_seq: Array = []
		for _step in max(0, rival.cell.y - approach_target.y):
			approach_seq.append(UP)
		await walk(rival, approach_seq)
		rival.face(UP)
	main.start_trainer_battle("OPP_RIVAL2", _SILPH_RIVAL_PARTY.get(_rival_st(), 9), "SilphCo7F:3,7")
	await main.battle.finished
	if not main.battle.won:
		main.cutscene_active = false
		return
	main.set_event("BEAT_SILPH_CO_RIVAL")
	if rival:
		rival.face(UP)
	await say("Oh ho!\nSo, you are ready\nfor BOSS ROCKET!\f%s: Good luck to\nyou!" % rn)
	if rival:
		if main.audio:
			main.audio.play_song("meetrival_alt")      # Music_RivalAlternateStart for his exit
		# scripts/SilphCo7F.asm RivalExitRightMovement / RivalWalkAroundPlayerMovement.
		var exit_seq: Array = [RIGHT, RIGHT] if at_lower else [LEFT, UP, UP, RIGHT, RIGHT, RIGHT, DOWN]
		await walk(rival, exit_seq)
		rival.set_shown(false)
		if main.audio:
...
## A grateful Silph worker on 7F gives you a LAPRAS.
func silph_lapras() -> void:
	main.cutscene_active = true
	main.modal = null
	if main.has_event("GOT_LAPRAS"):
		await say("Is our PRESIDENT\nOK?")
		main.cutscene_active = false
		return
	await say("Oh! Hi! You're\nnot a ROCKET! You\ncame to save us?\fI want you to\nhave this POKéMON\nfor saving us.")
	var to_pc: bool = main.player_party.size() >= 6       # GivePokemon: party full -> BILL's PC (gh #157)
	if to_pc:
		main.pc_box.append(main.make_mon("lapras", 15, []))
	else:
		main.player_party.append(main.make_mon("lapras", 15, []))
	if main.audio:
		main.audio.play_sfx("get_item1")
	if to_pc:
		# GivePokemon's full-party path prints SentToBoxText (give_pokemon.asm), not a "received"
		# line; the port has a single box named BILL's PC, so drop pokered's numbered "BOX N" (gh #157).
		await say("There's no more\nroom for POKéMON!\fLAPRAS was sent\nto BILL's PC!")
	else:
		await say("%s received\na LAPRAS!" % main.player_name)
	main.set_event("GOT_LAPRAS")
	main.cutscene_active = false


## Silph Co 11F boss: Giovanni #2. Beat him and he flees; the president then gives the MASTER BALL.
func giovanni_silph() -> void:
	main.cutscene_active = true
	main.modal = null
	var giovanni = main._npc_by_key("SPRITE_GIOVANNI@6,9")
	if giovanni:
		giovanni.face_to(main.player.cell)
	await say("Ah %s!\nSo we meet again!\fThe PRESIDENT and\nI are discussing a\nvital business\nproposition.\fKeep your nose out\nof grown-up\nmatters...\fOr, experience a\nworld of pain!" % main.player_name)
	main.start_trainer_battle("OPP_GIOVANNI", 2, "SilphCo11F:6,9")
	await main.battle.finished
	if not main.battle.won:
		main.cutscene_active = false
		return
	main.set_event("BEAT_SILPH_CO_GIOVANNI")
	await say("Arrgh!\fYou are stronger\nthan I expected!\fAgain, you ruined\nour plans for\nSILPH!\fBut TEAM ROCKET\nwon't give up!")
	# SilphCo11FGiovanniAfterBattleScript: GBFadeOutToBlack, TEAM ROCKET leaves the building
	# under the dark (the ShowObject/HideObject sweep — Giovanni AND this floor's rockets),
	# a beat, then GBFadeInFromBlack (gh #158). The other floors + Saffron's streets already
	# flip at load via their object_shown gates on the event set above.
...
## The rescued Silph president rewards you with the MASTER BALL (after Giovanni flees).
func silph_president() -> void:
	main.cutscene_active = true
	main.modal = null
	if main.has_event("GOT_MASTER_BALL"):
		await say("PRESIDENT: SILPH's\nfuture depends on\nyoung people\nlike you!")
		main.cutscene_active = false
		return
	await say("PRESIDENT: Thank\nyou for saving\nSILPH!\fI will never\nforget you saved\nus in our moment\nof peril!\fLet me give you\nthis as thanks!")
	if not await _gift("MASTER BALL"):
		return
	if main.audio:
		main.audio.play_sfx("get_key_item")
	await say("%s got a\nMASTER BALL!\fThat's the best\nBALL! It catches\nanything, no fail!" % main.player_name)
	main.set_event("GOT_MASTER_BALL")
	main.cutscene_active = false


## RocketHideoutB4FRocket3AfterBattleText: the beaten grunt admits he dropped the LIFT KEY, and only
## once the text is dismissed does the ball turn up beside him (CheckAndSetEvent -> ShowObject ITEM_5).
func rocket_drops_lift_key(npc) -> void:
	main.cutscene_active = true
...
## Rocket Hideout B4F boss: Giovanni guards the SILPH SCOPE; beat him and he steps aside.
func giovanni_hideout() -> void:
	...
	main.start_trainer_battle("OPP_GIOVANNI", 1, "RocketHideoutB4F:25,3")
	await main.battle.finished
	if not main.battle.won:
		main.cutscene_active = false
		return
...
# Player starter -> OPP_RIVAL3 party for the Champion battle (scripts/ChampionsRoom.asm).
const _CHAMPION_PARTY := {"squirtle": 1, "bulbasaur": 2, "charmander": 3}


## Walking into the Champion's room is the trigger, not talking to him: beating AGATHA arms
## `SCRIPT_CHAMPIONSROOM_PLAYER_ENTERS`, and the next load marches the player up the room's
## `RivalEntrance_RLEMovement` (UP 1, RIGHT 1, UP 3) straight into the last battle. Without it the rival
## can simply be walked around — the Hall of Fame stairs at (3,0)/(4,0) are reachable from the door.
func champion_entrance() -> void:
	main.cutscene_active = true
	main.modal = null
	# RivalEntrance_RLEMovement plays in REVERSE of its declaration — the simulated joypad
	# consumes wSimulatedJoypadStates from the end down (overworld.asm dec [hl]; gh #182) —
	# so UP1,RIGHT1,UP3 executes as UP×3, RIGHT, UP: up the left column, then across.
	await walk(main.player, [1, 1, 1, 3, 1])       # -> (4,3), facing the rival
	await champion_battle()


## The final battle: the rival is the POKéMON LEAGUE CHAMPION. Beat him -> the Hall of Fame.
func champion_battle() -> void:
	main.cutscene_active = true
	main.modal = null
	var rn: String = main.rival_name
	var pn: String = main.player_name
	var rival = main._npc_by_key("SPRITE_BLUE@4,2")
	if rival:
		rival.face_to(main.player.cell)
	if main.audio:
		main.audio.play_song("meetrival")
	await say("%s: Hey!\fI was looking\nforward to seeing\nyou, %s!\fMy rival should\nbe strong to keep\nme sharp!\fAnd now!\fI'm the POKéMON\nLEAGUE champion!\fDo you know what\nthat means?" % [rn, pn])
	var num: int = _CHAMPION_PARTY.get(_rival_st(), 3)
	main.start_trainer_battle("OPP_RIVAL3", num, "ChampionsRoom:4,2")
	await main.battle.finished
	if not main.battle.won:
		main.cutscene_active = false
		return
	main.set_event("BEAT_CHAMPION")
	# RivalDefeatedText (the end-battle line, rendered post-battle) then the overworld
	# _ChampionsRoomRivalAfterBattleText lament — two texts, not one truncated line (gh #179).
	await say("NO!\nThat can't be!\nYou beat my best!\fAfter all that\nwork to become\nLEAGUE champ?\fMy reign is over\nalready?\nIt's not fair!")
	await say("Why?\nWhy did I lose?\fI never made any\nmistakes raising\nmy POKéMON...\fDarn it! You're\nthe new POKéMON\nLEAGUE champion!\fAlthough I don't\nlike to admit it.")
	await champions_room_oak()
	main.cutscene_active = false


## The Hall of Fame ceremony: each party POKéMON is registered (pic + name/level), then credits.
## scripts/ChampionsRoom.asm, the post-battle half (gh #179): OAK arrives. His "OAK: <PLAYER>!"
## line sounds first, then he appears at the south door (ShowObject TOGGLE_CHAMPIONS_ROOM_OAK)
## and walks up beside the pair (OakEntranceAfterVictoryMovement, UP ×5). He congratulates the
## player naming the starter (GetMonName wPlayerStarter — the species, not the rival), turns on
## the rival and scolds him, calls the player along, exits north (UP ×2, HideObject), and the
## player follows him up the LEFT column, around the rival — the RLE list plays in reverse
## (gh #182) — onto the (3,0) door warp.
func champions_room_oak() -> void:
	var pn: String = main.player_name
	var rn: String = main.rival_name
	if main.audio:
		main.audio.play_song("cities1")               # Music_Cities1AlternateTempo
	await say("OAK: %s!" % pn)
	var oak = main._npc_by_key("SPRITE_OAK@3,7")
	var rival = main._npc_by_key("SPRITE_BLUE@4,2")
	if oak:
		oak.set_shown(true)                           # ShowObject TOGGLE_CHAMPIONS_ROOM_OAK
		await walk(oak, [UP, UP, UP, UP, UP])         # -> (3,2), stopping level with the rival
	main.player.face(LEFT)                            # PLAYER_DIR_LEFT
	if rival:
		rival.face(LEFT)
	if oak:
		oak.face(DOWN)
	await say("OAK: So, you won!\nCongratulations!\nYou're the new\nPOKéMON LEAGUE\nchampion!\fYou've grown up so\nmuch since you\nfirst left with\n%s!\f%s, you have\ncome of age!" % [main.player_starter.to_upper(), pn])
	if oak:
		oak.face(RIGHT)                               # turns on the rival
	await say("OAK: %s! I'm\ndisappointed!\fI came when I\nheard you beat\nthe ELITE FOUR!\fBut, when I got\nhere, you had\nalready lost!\f%s! Do you\nunderstand why\nyou lost?\fYou have forgotten\nto treat your\nPOKéMON with\ntrust and love!\fWithout them, you\nwill never become\na champ again!" % [rn, rn])
	if oak:
		oak.face(DOWN)
	await say("OAK: %s!\fYou understand\nthat your victory\nwas not just your\nown doing!\fThe bond you share\nwith your POKéMON\nis marvelous!\f%s!\nCome with me!" % [pn, pn])
	if oak:
		await walk(oak, [UP, UP])                     # OakExitChampionsRoomMovement
		oak.set_shown(false)                          # HideObject — he has gone on ahead
	# WalkToHallOfFame_RLEMovement (UP4, LEFT1) plays in REVERSE — the sim joypad consumes its
	# list from the end down (gh #182) — so the player sidesteps LEFT and follows OAK up the
	# LEFT column, AROUND the rival, onto the (3,0) door (its warp cuts the 4th UP).
	await walk(main.player, [LEFT, UP, UP, UP])
	await hall_of_fame()
...
## Silph Co. 9F's nurse silently heals before a white flash, unless Giovanni has been beaten
## (scripts/SilphCo9F.asm SilphCo9FNurseText: HealParty, fades, Delay3, conditional thank-you text).
func silph_co9f_nurse() -> void:
	main.cutscene_active = true
	main.modal = null
	if main.has_event("BEAT_SILPH_CO_GIOVANNI"):
		await say("Thank you so\nmuch!")
	else:
		await say("You look tired!\nYou should take a\nquick nap!")
		main.heal_party()
		await fade_out()
		await wait(3.0 / 60.0)
		await fade_in()
		await say("Don't give up!")
	main.cutscene_active = false
...
# ---- Oak's Parcel errand (scripts/ViridianMart.asm + scripts/OaksLab.asm) ---

const PARCEL := "OAK's PARCEL"


## Viridian Mart clerk hands over OAK's PARCEL on first visit (ViridianMartDefaultScript ->
## ViridianMartOaksParcelScript). The clerk calls from afar, the player walks to the counter,
## then receives the parcel. Fired from Main._on_map_loaded.
func viridian_mart_parcel() -> void:
	main.cutscene_active = true
	main.modal = null
	var clerk = main._npc_by_key("SPRITE_CLERK@0,5")
	await say("Hey! You came from\nPALLET TOWN?")
	if clerk:                                      # stand in front of the counter (not on it)
		await walk(main.player, main.find_path(main.player.cell, clerk.cell + Vector2i(2, 0)))
		main.player.face(LEFT)
		clerk.face_to(main.player.cell)
	await say("You know PROF.\nOAK, right?\fHis order came in.\nWill you take it\nto him?")
	if main.audio:
		main.audio.play_sfx("get_key_item")
	await say("%s got\nOAK's PARCEL!" % main.player_name)
	main.player_bag[PARCEL] = 1
	main.set_event("GOT_OAKS_PARCEL")
	main.cutscene_active = false


## Deliver OAK's PARCEL to Oak in his lab, then the Pokédex receipt (scripts/OaksLab.asm
## OaksLabOak1Text .got_parcel -> RivalArrivesAtOaksRequest -> OakGivesPokedex). Fired when the
## player talks to Oak while carrying the parcel.
func deliver_parcel() -> void:
	main.cutscene_active = true
	main.modal = null
	var pn: String = main.player_name
	var oak = main._npc_by_key("SPRITE_OAK@5,2")
	if oak:
		oak.face(DOWN)
	await say(("OAK: Oh, %s!\fHow is my old\nPOKéMON?" % pn)
		+ "\fWell, it seems to\nlike you a lot.\fYou must be\ntalented as a\nPOKéMON trainer!"
		+ "\fWhat? You have\nsomething for me?")
	main.player_bag.erase(PARCEL)
	if main.audio:
		main.audio.play_sfx("get_key_item")      # the fanfare plays with the "delivered" line
	await say("%s delivered\nOAK's PARCEL." % pn)
	await say("Ah! This is the\ncustom POKé BALL\nI ordered!\nThank you!")
	main.set_event("OAK_GOT_PARCEL")
	await pokedex_receipt()
	main.cutscene_active = false


## The rival rushes back in at Oak's request and both get the Pokédex (OaksLabRivalArrivesAt-
## OaksRequestScript + OaksLabOakGivesPokedexScript). Assumes cutscene_active is set.
func pokedex_receipt() -> void:
	var pn: String = main.player_name
	var rn: String = main.rival_name
	if main.audio:
		main.audio.play_song("meetrival_alt")      # Music_RivalAlternateStart for his arrival
	var rival = main._npc_by_key("SPRITE_BLUE@4,3")
	if rival:                                      # rival re-enters from the lab door and comes up to Oak
		rival.cell = Vector2i(4, 11)
		rival.position = Vector2(rival.cell * 16)
		rival.set_shown(true)
	await say("%s: Gramps!" % rn)
	if rival:
		await walk(rival, main.find_path(rival.cell, Vector2i(4, 3)))
		rival.face(UP)
	# The rival jingle keeps playing through the scene (the asm never reverts here; it
	# re-strikes Music_RivalAlternateStart when he leaves).
	await say("%s: What did\nyou call me for?" % rn)
	await say("OAK: Oh right! I\nhave a request\nof you two.")
	await say("On the desk there\nis my invention,\nPOKéDEX!\fIt automatically\nrecords data on\nPOKéMON you've\nseen or caught!\fIt's a hi-tech\nencyclopedia!")
	await say("OAK: %s and\n%s! Take\nthese with you!\f%s got\nPOKéDEX from OAK!" % [pn, rn, pn])
	for k in ["SPRITE_POKEDEX@2,1", "SPRITE_POKEDEX@3,1"]:
		var dex = main._npc_by_key(k)
		if dex:
			dex.set_shown(false)
	main.set_event("GOT_POKEDEX")
	await say("To make a complete\nguide on all the\nPOKéMON in the\nworld...\fThat was my dream!\fBut, I'm too old!\nI can't do it!\fSo, I want you two\nto fulfill my\ndream for me!\fGet moving, you\ntwo!\fThis is a great\nundertaking in\nPOKéMON history!")
	if rival:
		rival.face(RIGHT)
	await say(("%s: Alright\nGramps! Leave it\nall to me!" % rn)
		+ ("\f%s, I hate to\nsay it, but I\ndon't need you!" % pn)
	...
	if main.audio:
		main.audio.play_song("meetrival_alt")          # re-struck for his exit (Music_RivalAlternateStart)
	if rival:                                      # rival leaves out the lab door
		await walk(rival, main.find_path(rival.cell, Vector2i(4, 11)))
		rival.set_shown(false)
	if main.audio:
		main.audio.play_song("oakslab")            # the lab theme resumes once he's gone
	main.set_event("RIVAL_GOT_POKEDEX")
