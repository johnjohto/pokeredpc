...
| Flag | What it does |
|---|---|
| `--studio` | **Boot the Studio editor shell instead of the game** (ADR-020, gh #47–#50): one binary, two faces — the project browser (recents in `user://studio.cfg`), content-type + record panes (species/moves/items/trainers), and schema-generated record forms with inline validation and explicit canonical Save/Revert. Species adds project-asset sprite previews, a bounded type selector, and an ordered learnset table; trainers add an ordered multi-party builder. Open any project folder; never edit the extractor-owned `game/project` in place (copy it first — the d2 convention). `--studio-project=<dir>` opens a folder directly, skipping the browser. |
| `--studiotest` | Studio's smoke suite (grows per Phase-4 sub-issue): scratch-copies Kanto, opens it through the shell, checks the four content counts + bad-folder refusal + 515-record canonical identity sweep, then drives real generated controls through typed fields, `x-ref` pickers, valid save, invalid/dangling-ref refusal, missing-field repair, free-form `custom` JSON, array/optional add-remove, Revert, shell mounting, registry overrides, sprite/type/learnset/party widgets, and complete validated species/trainer saves. Headless. |
