...
| Flag | What it does |
|---|---|
| `--studio` | **Boot the Studio editor shell instead of the game** (ADR-020/021, gh #47–#52/#59): one binary, two faces — Studio disables the game's 160×144 pixel-art stretch and opens a themed native resizable 1280×800 desktop window (900×600 minimum), with a persistent top-bar UI-scale slider (80–200%, default 125%), project browser, content/map navigation, schema-generated forms, and isolated child-process Play-test. Format-2 maps open in the reference-shaped read-only tool/canvas/inspector workspace; format-1 Kanto maps name the pending migration instead of pretending to preview. Forms provide inline validation and canonical Save/Revert. Open any project folder; never edit extractor-owned `game/project` in place (copy it first). `--studio-project=<dir>` opens a folder directly. |
