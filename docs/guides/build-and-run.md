...
| `--eventtest` | The Event VM tracer (gh #39/#65, ADR-019/028): loader boot-refusals, the generic `EventMapScript` serving BluesHouse, the `visible` query behind `object_shown`, the authored Daisy TOWN MAP beat end-to-end, and the HatchScript event hatch (meaningful state queries, queued EventVM command, scalar result branch, direct-boot record checks, positioned source/reference refusals, and transactional runtime/missing-return rollback). Headless. |
...
| `--validate=<dir>` | v2 Core (gh #22 + gh #57, ADR-027): lint any project directory (res:// or OS path) — format/schema validation plus the map/story softlock lints (blocking objects, unbacked event doors, standability, reachability, link mismatches). Reviewed warnings clear only via named entries with reasons in `data/lint_suppressions.json`; errors are never suppressible. Exit 0 only when no unreviewed diagnostics remain. Headless; the determinism workflow gates on it. |
| `--studio-shot=<file>` | Studio (gh #61): windowed visual capture — opens `--studio-project=<dir>`, mounts the first species record's form, saves the window to `<file>`, quits. For docs/review, not a gate (headless viewport readback is empty). |
...
| `--exprtest` | v2 expression languages (gh #35/#64, ADR-018 §3/ADR-028): FormulaExpr unit semantics and the ~1.3k-vector Gen-1 kernel equivalence sweep, plus the HatchScript Core smoke for statements, strings/booleans, host-call sandboxing, positioned errors, loop-budget termination, and repeated-run determinism. Headless. |
...
| `--savetest` | Save/load round-trip (money/bag/party/flags plus HatchScript event-variable scalar types) + overworld poison tick. Headless. |
