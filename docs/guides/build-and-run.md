...
| Flag | What it does |
|---|---|
| `--studio` | **Boot the Studio editor shell instead of the game** (ADR-020, gh #47–#51/#59): one binary, two faces — Studio disables the game's 160×144 pixel-art stretch and opens a native resizable 1280×800 desktop window (900×600 minimum), with the project browser (recents in `user://studio.cfg`), content-type + record panes (species/moves/items/trainers), schema-generated forms, and a Play-test button that launches the Engine as a separate child on the saved project with an isolated per-project save slot. Forms provide inline validation and canonical Save/Revert; species adds project-asset sprite previews, a bounded type selector, and an ordered learnset table; trainers add an ordered multi-party builder. Open any project folder; never edit the extractor-owned `game/project` in place (copy it first — the d2 convention). `--studio-project=<dir>` opens a folder directly, skipping the browser. |
| `--studiotest` | The complete Phase-4 Studio gate: scratch-copies Kanto, opens it through the shell, checks the four content counts + bad-folder refusal, actually load/re-saves all 515 editable records byte-identically, drives generated and custom widgets through refused and valid saves/Revert, restores the fixture byte-for-byte, then launches a separate headless Engine probe and verifies its project/save handshake and clean exit. Headless. |
