...
| `--clubtest` | The Cable Club attendant (gh #5). Alone: every single-instance refusal/timeout path lands back cleanly. With `--clubhost` / `--clubjoin [--port=N] [--tamper=X]`: one side of the full in-game link flow (driven in pairs by `tools/linktest.py`). |
