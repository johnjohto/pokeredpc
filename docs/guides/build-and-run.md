...
| Flag | What it does |
|---|---|
| `--studio` | **Boot the Studio editor shell instead of the game** (ADR-020, gh #47): one binary, two faces — the project browser (recents in `user://studio.cfg`), the content-type sidebar (species/moves/items/trainers), and the record list. Open any project folder; never edit the extractor-owned `game/project` in place (copy it first — the d2 convention). `--studio-project=<dir>` opens a folder directly, skipping the browser. |
| `--studiotest` | Studio's smoke suite (grows per Phase-4 sub-issue): scratch-copies the Kanto project, opens it through the shell, asserts the four content types' counts, and proves the bad-folder refusal is loud and non-fatal. Headless. |
