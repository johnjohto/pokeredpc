...
| `--rulesettest` | v2 Phase 2 (gh #31, ADR-018): the ruleset seam — boot must resolve the manifest's ruleset through the registry, an unknown name must refuse, and the seam's type resolver must match the raw project chart over the full type cross-product (the Types tracer bullet). Headless. |
