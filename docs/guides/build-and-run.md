...
| `--rulesettest` | v2 Phase 2 (gh #31/#32, ADR-018): the ruleset seam — boot must resolve the manifest's ruleset through the registry, an unknown name must refuse, the seam's type resolver must match the raw project chart over the full type cross-product (the Types tracer bullet), and the formula kernels answer Gen-1's book values (exp curves, Mew's 298/403, stage table, damage core, the crit byte, the 1/256 sure-miss, MASTER BALL). Headless. |
